...
The original case was faceswap: `faceswap_model` and `faceswap_pixel_boost` shipped wired into
the append path only, so a job created with them in `first_segment` silently dropped both and
fell back to daemon defaults -- invisible unless you queried the row, because the defaults
happened to be what was wanted. Faceswap is gone (#209) and that specific test with it; the
general guards below outlived it, and caught the same shape twice more since.
